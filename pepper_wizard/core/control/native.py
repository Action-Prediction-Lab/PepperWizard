import numpy as np
from .base import ExponentialSmoother, AlphaBetaEstimator, TrapezoidalScheduler
...
class NativeController:
    ...
    def update(self, error_x, error_y, current_yaw, current_pitch, dt=0.01, timestamp=None):
        native_cfg = self.config.get("native", {})
        ...
        if error_x is not None and error_y is not None and current_yaw is not None and current_pitch is not None:
            ...
            # Velocity Estimation
            self.estimator_yaw.update(raw_target_yaw, timestamp)
            self.estimator_pitch.update(raw_target_pitch, timestamp)
        else:
            # Ghost Pursuit / Propagation
            if self.smoother_yaw.value is not None:
                # Use a safe dt for propagation
                p_dt = min(dt, 0.05)
                v_yaw = self.estimator_yaw.propagate(p_dt, kd_v)
                v_pitch = self.estimator_pitch.propagate(p_dt, kd_v)
        ...
        if target_pos_yaw is not None and current_yaw is not None:
            ...
            final_yaw = self.scheduler_yaw.update(target_pos_yaw, current_yaw, inner_dt, self.estimator_yaw.velocity)
            final_pitch = self.scheduler_pitch.update(target_pos_pitch, current_pitch, inner_dt, self.estimator_pitch.velocity)
